...
export function DeleteReviewButton({ reviewId }: { reviewId: string }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="inline-flex items-center gap-1 rounded-lg border border-red-500/30 bg-red-500/10 px-2.5 py-1 text-xs font-semibold text-red-400 hover:bg-red-500/20 disabled:opacity-50"
    >
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-4 rounded-xl border border-white/10 bg-surface/50 p-4 space-y-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
        <div>
          <label className="block text-xs font-medium text-muted mb-1">TMDB ID *</label>
          <input
            name="tmdbId"
            type="number"
            required
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <button
          type="submit"
          disabled={loading}
          className="ml-auto inline-flex items-center gap-1.5 rounded-lg bg-accent px-4 py-2 text-xs font-semibold text-background transition hover:brightness-110 disabled:opacity-50"
        >
